...
const QUICK_SEARCHES = ["Carl", "Carl Hancock", "yabasto@yahoo.co.uk", "3655", "CAR-1781959748"];
...
function FieldGrid({ rows }: { rows: Array<{ label: string; value: string | null | boolean | number }> }) {
  return (
...
export function TraceLeadPanel({
  ...
  const [query, setQuery] = useState("Carl Hancock");
  ...
  const runTrace = useCallback(async (q?: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/jarvis/trace-lead?q=${encodeURIComponent(term)}`);
      const json = (await res.json()) as { trace?: LeadTraceReport; error?: string };
  ...
  const runRepair = useCallback(async () => {
    const messageId = trace?.step4_stored.gmail_message_id ?? trace?.step1_gmail.message_id;
    if (!messageId) return;
    setBusy("repair");
    setError(null);
    try {
  ...
  return (
    <div className="mt-6 rounded-lg border border-amber-500/25 bg-amber-950/10 p-4">
      <p className="mb-1 text-xs uppercase tracking-widest text-amber-300/90">
        Trace Lead (end-to-end diagnostic)
      </p>
      <p className="mb-3 text-xs text-slate-500">
        Single-lead pipeline check: Gmail → parse → stored ledger → i-MVE match → ROI → area table.
      </p>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        <input
          ...
          placeholder="Carl / yabasto@yahoo.co.uk / 3655"
        ...
        {canRepair && (
          ...
          >
            {busy === "repair" ? "Repairing…" : "Repair this lead"}
          </button>
        )}
      </div>
      ...
      {trace && (
        <div className="space-y-3">
          ...
          <Step n={1} title="Raw Gmail message">
            <FieldGrid
              rows={[
                ...
                { label: "Search used", value: trace.step1_gmail.search_used },
          ...
          <Step n={4} title="Stored CMM ledger record">
            <FieldGrid
              rows={[
                ...
                { label: "Full name", value: trace.step4_stored.has_full_name },
                { label: "Has email", value: trace.step4_stored.has_email },
                { label: "Has phone", value: trace.step4_stored.has_phone },
          ...
          <Step n={5} title="i-MVE job (e.g. 3655)">
            <FieldGrid
              rows={[
          ...
          <Step n={6} title="Match score (CMM lead ↔ i-MVE job)">
            <FieldGrid
              rows={[
                ...
                { label: "Stored i-MVE job", value: trace.step6_match.stored_imve_job_id },
          ...
          <Step n={7} title="ROI eligibility + area table">
            <FieldGrid
              rows={[
                { label: "Included in ROI", value: trace.step7_roi.included },
                { label: "Exclusion reason", value: trace.step7_roi.exclusion_reason },
                { label: "Match status", value: trace.step7_roi.match_status },
                { label: "Area", value: trace.step7_roi.area },
